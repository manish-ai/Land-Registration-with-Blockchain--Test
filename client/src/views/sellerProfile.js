    ...
    componentDidMount = async () => {
        try{
            ...
            const sellerTableEl = (<>
              ...
              {infoField('Name', seller[0])}
              <div style={{ display: 'flex', gap: 16 }}>
                <div style={{ flex: 1 }}>{infoField('Age', seller[1])}</div>
                <div style={{ flex: 1 }}>{infoField('Lands Owned', seller[2])}</div>
              </div>
              {infoField('Verification ID', seller[3], true)}
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.8, color: '#9a9a9a', marginBottom: 4 }}>Document</div>
                <a href={`http://localhost:4002/api/files/${seller[4]}`} target="_blank" rel="noreferrer"
                   style={{ color: '#1a5276', fontWeight: 600, fontSize: 13 }}>View Uploaded Document →</a>
              </div>
            </>);
            this.setState({ sellerTable: sellerTableEl, verification: statusBadge });
                                ...
                                <Card>
                                    <CardHeader>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <h5 className="title" style={{ margin: 0 }}>Seller Profile</h5>
